      ...
      return (args) -> {
         ...
         while(true) {
            ...
            try {
               ...
               System.out.println("VisitCount : " + vc + "VisitCount2 : " + vc2);

               if(vc >= 4 && cnt<=2) {
                  System.out.println("New Web Server starts at port= " + (port+cnt));
                  String x = "start /min /B java -jar lab3-0.0.1-SNAPSHOT.jar --server.port=" + String.valueOf(port + cnt);
                  ...
                  if(acc >= 10) {
                     acc = 0;
                     if(threads.size() > 0) {
                        System.out.println("Web Server Service stop at port= " + (port + threads.size() -1));
...
class MyThread implements Runnable {
   ...
   public void run() {
      ...
      String username = "yello";
      String host = "localhost";
      int port = 22; // (SSH서버포트번호)
      String password = "rkdus360";
